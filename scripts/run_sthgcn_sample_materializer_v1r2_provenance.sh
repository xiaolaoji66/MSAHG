...
output=${STHGCN_V1R2_OUTPUT_ROOT:?STHGCN_V1R2_OUTPUT_ROOT is required}
audit_output=${STHGCN_V1R2_AUDIT_OUTPUT:?STHGCN_V1R2_AUDIT_OUTPUT is required}
registry="$repo_root/reproduction/STHGCN_SAMPLE_MATERIALIZER_V1R2_PROVENANCE_REGISTRY.json"
export PYTHONDONTWRITEBYTECODE=1

"$python_bin" "$repo_root/reproduction/sthgcn_sample_materializer_v1r2_provenance.py" \
  --sthgcn-root "$sthgcn_root" \
  --v1-output-root "$v1_output" \
  --registry "$registry" \
  --output-root "$output"

"$python_bin" "$repo_root/reproduction/audit_sthgcn_sample_materializer_v1r2_provenance.py" \
  --output-root "$output" \
  --v1-output-root "$v1_output" \
...
echo "STHGCN_SAMPLE_MATERIALIZER_V1R2_PROVENANCE=PASS"
echo "OUTPUT=$output"
