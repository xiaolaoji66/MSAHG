#!/usr/bin/env python3
"""Add-only original-split provenance repair for STHGCN Materializer V1."""
...
REGISTRATION_COMMIT = "be5efdc1ed56b6628dde82224eaf6df2abfec1cf"
PARENT_CONTRACT_COMMIT = "cccf03c605bb3b1d7374e34a5eac9c247ea3b48b"
V1_RESULT_COMMIT = "9e67e05875540cc87a21e15c4ff3623400c6ab49"
STHGCN_COMMIT = "27b595846d29019799485985bff49f9ed02c4ade"
REGISTRY_SHA256 = "cfb573c2706fbea0870e001103b0c34dab992a62a970fb79764b953d68a7b45c"
...
def verify_registry(registry_path):
    ...
    if sha256_file(path) != REGISTRY_SHA256:
        raise ProvenanceError("V1R1 registry identity mismatch")
    with open(str(path), "r", encoding="utf-8") as handle:
        registry = json.load(handle)
    if registry.get("parent_commit") != PARENT_CONTRACT_COMMIT:
        raise ProvenanceError("V1R1 parent contract identity mismatch")
    if registry.get("v1_data_sha256") != EXPECTED_V1_SHA256:
        raise ProvenanceError("V1 data hash registry mismatch")
    if tuple(registry.get("gates", ())) != GATE_NAMES:
        raise ProvenanceError("V1R1 gate set mismatch")
    return registry
...
def _prepare_dataset(v1, dataset, archive_path, v1_output_root, output_root):
    ...
    replay_parent = Path(tempfile.mkdtemp(prefix="sthgcn_v1r1_replay_", dir=str(output_root.parent)))
    try:
        replay_hashes = _write_replay(v1, replay_parent, sample, eligible, mappings)
        if replay_hashes != EXPECTED_V1_SHA256[dataset]:
            raise ProvenanceError("V1 replay identity mismatch for " + dataset)
    finally:
        shutil.rmtree(str(replay_parent), ignore_errors=True)
...
def _finalize_dataset(prepared, implementation_commit, synthetic_witness):
    ...
    receipt = {
        "schema": "msahg.sthgcn-sample-materializer-v1r1-provenance-receipt.v1",
        "dataset": dataset,
        "decision": decision,
...
def _write_failure(output_root, error):
    ...
    if path.exists():
        return
    receipt = {
        "schema": "msahg.sthgcn-sample-materializer-v1r1-provenance-failure.v1",
...
def run(args, output_root):
    ...
    receipts = [
        _finalize_dataset(item, runtime["implementation_commit"], synthetic)
        for item in prepared
    ]
    summary = {
        "schema": "msahg.sthgcn-sample-materializer-v1r1-provenance-summary.v1",
...
def main(argv=None):
    ...
    try:
        ...
        return 2
    print("STHGCN_SAMPLE_MATERIALIZER_V1R1_PROVENANCE={}".format(summary["decision"]))
